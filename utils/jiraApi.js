// utils/jiraApi.js - Converted for importScripts compatibility
(function(global) {
    'use strict';
    ...
    // Exported functions
    const jiraApi = {
        async getCreateMetadata(credentials, projectKey, issueTypeName) {
        ...
        async getAllJiraIssueTypes(credentials) {
            if (!credentials || !credentials.baseUrl) {
        ...
        async getJiraIssueTypesForProject(credentials, projectIdOrKey) {
            if (!projectIdOrKey) {
            ...
            try {
                const allIssueTypes = await this.getAllJiraIssueTypes(credentials);
        ...
        async createJiraIssue(credentials, issueData, availableFields = null) {
        ...
        async addJiraAttachment(credentials, issueIdOrKey, attachmentBlob, attachmentFilename) {
    ...
    global.jiraApi = jiraApi;

    console.log('jiraApi.js loaded (non-module version)');
})(self);